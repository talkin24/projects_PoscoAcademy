...
f = open('/home/pirl/A3_/proj_bigdata/hanbin/movieURL_moviemetaExclusive_erased.txt', 'r')
tabs = f.readlines()
f.close()
f = open('error_0~199.txt', 'w')
i = 0
for tab in tabs[0:200]:
    try:
        i += 1
        print(i)
        url = tab
        ...
        if len(text_v) != len(text_k):
            f.write("len(text_v) != len(text_k) {}".format(tab))
            pass
        else:
...
df.to_csv('/home/pirl/A3_/proj_bigdata/hanbin/numbers0~199.csv', header=True, index=False)
